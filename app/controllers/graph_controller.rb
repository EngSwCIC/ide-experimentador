...
class GraphController < ApplicationController
    ...
    def skillsGet
        ...
        #
        #   1) "86.29, [info], nurse, sync, received-request, (status=sending-request)"
        #   # => { "time" => 86.29, "message" => Sending message to nurse}
        #
        #   2) "86.29, [info], nurse, sync, request-sent, (status=waiting)"
        #   # => { "time" => 86.29, "message" => Waiting the message get to nurse}
        #
        #   3) "86.29, [info], nurse, sync, wait-message, (status=message-received)"
        ...
        @trials.each do |trial|
            ...
            json_id << {"id" => trial.id, "lines" => json_data}  
        end
        pretty_jsonData = JSON.pretty_generate(json_id)
        render json: pretty_jsonData
